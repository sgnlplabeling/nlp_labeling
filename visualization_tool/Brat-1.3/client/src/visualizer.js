...
var Visualizer = (function($, window, undefined) {
    var fontLoadTimeout = 500; // 5 seconds
    ...
    var Visualizer = function(dispatcher, svgId, webFontURLs) {
      ...
      // 2017.09.14 chrome error 발생, 삭제  @jhseong
...
//            rowBoxHeight -= 5; // XXX TODO HACK: why -5?
          ...
          // 2017.11.14 number40 배경크기를 수정함에 있어 태깅 위치 조정
          translate(row, 0, rowY - 5);
          y += Configuration.visual.margin.y;
        });
        y += Configuration.visual.margin.y;
        
        // 2017.11.22 GOOD인 라인을 찾아 스타일을 변경
        if(labelGrade != null && labelGrade != 'undefined') {
        	var labelGrades = labelGrade.split('|');
            
            for ( var i in labelGrades ) {
            	var line = labelGrades[i].substr(0, labelGrades[i].length - 1);
...
Util.profileStart('chunkFinish');
        ...
        $.each(data.chunks, function(chunkNo, chunk) {
          ...
          // 2017.11.08 number40 태그 된 단어에 ANN_ID를 아이디로 부여하도록 추가
          var chkId= '';

          for(var idx=0; idx < currentChunk.fragments.length; idx++) {
            if(idx == currentChunk.fragments.length-1) {
              chkId += currentChunk.fragments[idx].span.id;
            } else {
              chkId += currentChunk.fragments[idx].span.id + "_";
            }
          }
          ...
          sentenceText.span(chunk.text + nextSpace, {
            x: chunk.textX,
            // 2017.11.14 number40 배경크기를 수정함에 있어 텍스트 위치 조정
            y: chunk.row.textY - 5,
            'data-chunk-id': chunk.index,
            id : chkId // 2017.11.08 number40 태그 된 단어에 ANN_ID를 아이디로 부여하도록 추가
          });
          ...
          if (chunk.fragments.length) {
            ...
            for(var i=0; i<chunk.fragments.length; i++) {
              ...
              fragment.highlightPos = {
                  ...
                  // 2017.11.14 number40 배경크기를 수정함에 있어 하이라이팅 위치 조정
                  y: chunk.row.textY + sizes.texts.y + yShrink + yStartTweak - 5,
...
Util.profileStart('finish');
        ...
        $svg.width(canvasWidth-2); /**@coreawin div안에 꽉 차서 가로 스크롤이 생겨서 가로 스크롤이 생기지 않게 한다. TODO 확대 축소시 text 줄바꿈 처리*/
        $svg.height(y);
        $svgDiv.height(y);
