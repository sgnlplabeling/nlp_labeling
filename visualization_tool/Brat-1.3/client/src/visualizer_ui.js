...
//         if (show === visible) return false;
	...
	*/
      ...
      /**@coreawin 로그인 실행 모듈. <br>
       * 세션에 계정정보를 저장하고 있다가, brat 페이지가 호출 될때 document.ready 상태가 되면 아래 코드를 실행 시키고,<br>
       * 해당 계정으로 아래의 정보를 만드어 준다.<br>
       * ex) 로그인 성고시에 response 내용. {action: "login", user: "diquest", password: "11111", protocol: 1}
       * */
      var authFormSubmit = function(evt) {
        ...
        dispatcher.post('ajax', [{
          ...
          function(response) {
        	  /**@coreawin 아래의 관리도구에서 들어온 계정을 강제로 세팅해 준다.
        	   * 즉 response를 여기에 맞게 설정해 준다. (계정은 관리도구 계정만 관리) 
        	   * 세션에서 읽어야 재로딩할때도 다시 이 모듈을 호출할 수 있음..*/
              if (response.exception) {
