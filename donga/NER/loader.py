...
def mor_mapping(sentences):
    ...
    print("Found %i unique named entity pos" % len(dico))
...
def prepare_dataset(sentences, word_to_id, char_to_id, tag_to_id, mor_to_id, lower=True):
    """
    ...
    for s in sentences:
        ...
        mor = [mor_to_id[w[2] if w[2] in mor_to_id else '<UNK>'] for w in s]
        data.append({
            'str_words': str_words,
            'words': words,
            'chars': chars,
