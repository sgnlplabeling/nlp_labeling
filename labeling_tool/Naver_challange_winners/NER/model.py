...
class Model(object):
	def __init__(self, config, word_to_id, pumsa_to_id, char_to_id, tag_to_id, embedding_matrix, ner_morph_tag=None):
		...
		self.ner_morph_tag = ner_morph_tag
		...
		if config["task"]=="NER":
			self.ner_dicts = tf.placeholder(dtype=tf.float32,
											shape=[None, None, 15],
											name="ner_dicts"
											)
		...
		if config["elmo"]:
			self.elmo1 = tf.nn.dropout(self.elmo1, self.dropout)
			self.elmo4 = tf.nn.dropout(self.elmo4, self.dropout)
		...
		if config["task"] == "NER":
			word_encoded = tf.concat([word_embed, self.ner_dicts], axis=-1)
	...
	def embedding_layer(self, word_inputs1, word_inputs2, word_inputs3, word_inputs4,
		...
		with tf.variable_scope("char_embedding", reuse=tf.AUTO_REUSE), tf.device('/cpu:0'):
			...
			char_embed = tf.reshape(output, shape=[-1, s[1], self.config["char_dim"]])
	...
	def create_feed_dict(self, sess, context_embeddings_op, elmo_context, elmo_ids, is_train, batch, elmo_dict=None):
		"""
		:param is_train: Flag, True for train batch
		:param batch: list train/evaluate data 
		:return: structured data to feed
		"""
		words1, words2, words3, words4, pumsas1, pumsas2, pumsas3, pumsas4, elmo, ner_dicts, w1_idxs, w4_idxs, chars, tags = batch
		...
		if self.config["task"] == "NER":
			feed_dict[self.ner_dicts] = np.array(ner_dicts)
		'''
		print ('chars')
		print (chars)
		print ('after chars')
	...
	def run_step(self, sess, context_embeddings_op, elmo_context, elmo_ids, is_train, batch, elmo_dict=None, ner_morph_tag=None):
		"""
		...
		"""
		feed_dict = self.create_feed_dict(sess, context_embeddings_op, elmo_context, elmo_ids, is_train, batch, elmo_dict=elmo_dict)
	...
	def evaluate_lines(self, sess, context_embeddings_op, elmo_context, elmo_ids, ner_morph_tag, inputs, id_to_tag):
		trans = self.trans.eval(session=sess)
		lengths, scores = self.run_step(sess, context_embeddings_op, elmo_context, elmo_ids, False, inputs, ner_morph_tag=ner_morph_tag)
		batch_paths = self.decode(scores, lengths, trans)
