...
def prepare_dataset(dataset, word_to_id, pumsa_to_id, char_to_id, tag_to_id, elmo_dict, config, train=True, ner_morph_tag=None):
	"""
	데이터셋 전처리를 수행한다.
	...
	for idx, sen in enumerate(dataset):
		...
		if config["task"] == "NER":
			token_dict = [_search_index_by_dict(ner_morph_tag, token, len(tag_to_id)) for token in sen[13]]
		else:
			token_dict=None
		...
		data.append([word_ids1, word_ids2, word_ids3, word_ids4,
					 pumsa_ids1, pumsa_ids2, pumsa_ids3, pumsa_ids4, fw_elmo_input, token_dict, sen[10], sen[11], chars, tag_ids])
