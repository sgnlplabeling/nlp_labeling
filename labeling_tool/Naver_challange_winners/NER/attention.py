...
def multihead_attention(query, memory, bias, key_size, value_size, output_size,
    ...
    with tf.variable_scope(scope, default_name="multihead_attention",
        ...
        if memory is None:
            ...
            combined = linear(query, size, True, True,
                              scope="qkv_transform")
            q, k, v = tf.split(combined, [key_size, key_size, value_size],
                               axis=axis)
        else:
            q = linear(query, key_size, True,
                       scope="q_transform")
            combined = linear(memory, key_size + value_size, True, scope="kv_transform")
        ...
        x = linear(x, output_size, True, scope="output_transform")
        return x
