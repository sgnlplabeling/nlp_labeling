...
def parse_args():
	...
	flags.DEFINE_integer("word_dim",        100, 	"Embedding size for words")
	...
	flags.DEFINE_integer("char_lstm_dim",	50,		"Num of hidden units in char LSTM")
	...
	flags.DEFINE_float("elmo_dropout", 	0.5, "Elmo Dropout rate")
	...
	# flags.DEFINE_string("elmo_weights", "/home/nlpgpu3/data/cm/nlp-challenge/missions/srl/data/elmo/weight/real_tmp_srl_weights.hdf5", "Path for elmo weights")
	...
	# flags.DEFINE_string("necessary", "necessary_SRL.pkl", "file for maps")

	# 이 부분은 NSML 내부의 폴더 접근
	flags.DEFINE_string("elmo_options", "/pretrained_data/SRL/elmo/weight/options.json", "Path for elmo options")
	flags.DEFINE_string("elmo_weights", "/pretrained_data/SRL/elmo/weight/real_tmp_srl_weights.hdf5", "Path for elmo weights")
	flags.DEFINE_string("elmo_dict",    "/pretrained_data/SRL/elmo/dict/final_char_dict.txt", "Path for elmo char_dict")
	flags.DEFINE_string("necessary", "/pretrained_data/SRL/necessary_SRL.pkl", "file for maps")
	...
	flags.DEFINE_string("task", "SRL", "NER or SRL")
...
def config_model():
	...
	config["elmo_dropout_keep"] = 1.0 - FLAGS.elmo_dropout
...
def bind_model(sess, context_embeddings_op, elmo_context, elmo_ids, FLAGS):
	def save(dir_path, *args):
		os.makedirs(dir_path, exist_ok=True)
		saver = tf.train.Saver()
		saver.save(sess, os.path.join(dir_path, 'model'))

		# with open(os.path.join(dir_path, FLAGS.necessary), "wb") as f:
		# 	pickle.dump([word_to_id, id_to_word, pumsa_to_id, id_to_pumsa, char_to_id, id_to_char, tag_to_id, id_to_tag, pre_to_id, id_to_pre, suf_to_id, id_to_suf], f)
	...
	def load(dir_path, *args):
		...
		with open(os.path.join(dir_path, FLAGS.necessary), "rb") as f:
			_, _, _, _, char_to_id, _, _, id_to_tag, _, _, _, _ = pickle.load(f)
	...
	def infer(sentences, **kwargs):
		...
		for idx in range(0, len(reformed_sentences), 100):
			...
			results.extend(model.evaluate_lines(sess, context_embeddings_op, elmo_context, elmo_ids,
												inputs_from_sentences(komoran, reformed_sentence, word_to_id, pumsa_to_id, char_to_id,
																	  elmo_dict, FLAGS.max_char_length), id_to_tag))
...
def train(sess, context_embeddings_op, elmo_context, elmo_ids, config):
	...
	for epoch in range(FLAGS.max_epoch):
		for batch in train_manager.iter_batch(shuffle=True):
			step, batch_loss = model.run_step(sess, context_embeddings_op, elmo_context, elmo_ids, True, batch)
...
def evaluate_cli(model, context_embeddings_op, elmo_context, elmo_ids):
	...
	with open(FLAGS.necessary, "rb") as f:
		word_to_id, id_to_word, pumsa_to_id, id_to_pumsa, char_to_id, id_to_char, tag_to_id, id_to_tag, pre_to_id, id_to_pre, suf_to_id, id_to_suf = pickle.load(
			f)
	...
	while True:
		...
		for idx in range(0, len(line), 5):
			l = line[idx:idx+2]
			results.extend(model.evaluate_lines(sess, context_embeddings_op, elmo_context, elmo_ids, inputs_from_sentences(komoran, l, word_to_id, pumsa_to_id, char_to_id, elmo_dict, FLAGS.max_char_length), id_to_tag))
		print(results)
...
if __name__ == '__main__':
	...
	with open(FLAGS.necessary, 'rb') as f:
		word_to_id, id_to_word, pumsa_to_id, id_to_pumsa, char_to_id, id_to_char, tag_to_id, id_to_tag, pre_to_id, id_to_pre, suf_to_id, id_to_suf = pickle.load(f)
	...
	bind_model(sess, context_embeddings_op, elmo_context, elmo_ids, FLAGS)
	...
	if FLAGS.mode == 'train':
		...
		train_data = prepare_dataset(train_dataset, word_to_id, pumsa_to_id, char_to_id, tag_to_id, elmo_dict, config)
		dev_data = prepare_dataset(dev_dataset, word_to_id, pumsa_to_id, char_to_id, tag_to_id, elmo_dict, config)
