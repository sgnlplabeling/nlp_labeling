...
def prepare_dataset(dataset, word2idx, pumsa2idx, char2idx, lemma2idx, label2idx, ELMo_dict):
        distance2idx = {-6: 0, -5: 1, -4: 2, -3: 3, -2: 4, -1: 5, 0: 6,1: 7, 2: 8, 3: 9, 4: 10, 5: 11, 6: 12}

        data = []

        bc = BertClient()

        for idx, sen in enumerate(zip(dataset[0], dataset[1], dataset[2], dataset[3], dataset[4], dataset[5], dataset[6], dataset[7], \
                ...
                word_bert1 = None
                for idx, s in enumerate(sen[0]):
                    if s == '':
                        sen[0][idx] = '<UNK>'
                if sen[0] == '':
                    word_bert1 = bc.encode([['<UNK>', '<UNK>']], is_tokenized=True)
                else:
                    bert_sen = []
                    bert_sen = list(sen[0])
                    word_bert1 = bc.encode([bert_sen], is_tokenized=True)
                word_bert1 = word_bert1[0]
                ...
                data.append([word_ids1, word_ids2, word_ids3, word_ids4, pumsa_ids1, pumsa_ids2, pumsa_ids3, pumsa_ids4, \
                        ...
                        predicate_word_ids1, predicate_word_ids2, predicate_pumsa_ids1, predicate_pumsa_ids2,predicate_distances, sen[19], word_bert1])
