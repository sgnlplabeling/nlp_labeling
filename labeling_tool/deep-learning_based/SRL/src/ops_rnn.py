...
def bidirectional_dynamic_rnn(cell_fw, cell_bw, inputs, sequence_length=None,
        ...
        swap_memory=False, time_major=False, scope=None, return_all_states=None):
    ...
    """
    #if not _like_rnncell(type(cell_fw), cell_fw):
    #raise TypeError("cell_fw must be an instance of RNNCell")
    #if not _like_rnncell(type(cell_bw), cell_bw):
    #raise TypeError("cell_bw must be an instance of RNNCell")
    _like_rnncell(type(cell_fw), cell_fw)
    _like_rnncell(type(cell_bw), cell_bw)

    with vs.variable_scope(scope or "bidirectional_rnn"):
    # Forward direction
        with vs.variable_scope("fw") as fw_scope:
            output_fw, output_state_fw, each_state_fw = dynamic_rnn(
                    cell=cell_fw, inputs=inputs, sequence_length=sequence_length,
                    initial_state=initial_state_fw, dtype=dtype,
                    parallel_iterations=parallel_iterations, swap_memory=swap_memory,
                    time_major=time_major, scope=fw_scope, return_all_states=return_all_states)
    ...
    with vs.variable_scope("bw") as bw_scope:
      ...
      tmp, output_state_bw, tmp_state = dynamic_rnn(
          cell=cell_bw, inputs=inputs_reverse, sequence_length=sequence_length,
          initial_state=initial_state_bw, dtype=dtype,
          parallel_iterations=parallel_iterations, swap_memory=swap_memory,
          time_major=time_major, scope=bw_scope, return_all_states=return_all_states)

    output_bw = _reverse(
        tmp, seq_lengths=sequence_length,
        seq_dim=time_dim, batch_dim=batch_dim)
    each_state_bw = _reverse(
        tmp_state, seq_lengths=sequence_length,
        seq_dim=time_dim, batch_dim=batch_dim)

    outputs = (output_fw, output_bw)
    output_states = (output_state_fw, output_state_bw)
    each_states = (each_state_fw, each_state_bw)

    return (outputs, output_states, each_states)


@tf_export("nn.dynamic_rnn")
def dynamic_rnn(cell, inputs, sequence_length=None, initial_state=None,
        dtype=None, parallel_iterations=None, swap_memory=False,
        time_major=False, scope=None, return_all_states=None):
    #if not _like_rnncell(type(cell),cell):
        #raise TypeError("cell must be an instance of RNNCell")
    _like_rnncell(type(cell),cell)
    ...
    if is_in_graph_mode.IS_IN_GRAPH_MODE() and sequence_length is not None:
    ...
    (outputs, final_state, each_state) = _dynamic_rnn_loop(
            cell,
            inputs,
            state,
    ...
    if not time_major:
        ...
        each_state = nest.map_structure(_transpose_batch_time, each_state)
    return (outputs, final_state, each_state)
...
def _dynamic_rnn_loop(cell,
  ...
  in_graph_mode = is_in_graph_mode.IS_IN_GRAPH_MODE()  #context.in_graph_mode()
  if in_graph_mode:
    output_ta = tuple(
        _create_ta(
            "output_%d" % i,
  ...
  def _time_step(time, output_ta_t, state, *args):
    """Take a time step of the dynamic R
    ...
    if in_graph_mode:
    #TODO new_state를 잘 append해서 관리하면 될듯요~~~
      output_ta_t = tuple(ta.write(time, out) for ta, out in zip(output_ta_t, output))
      state_ta_t = tuple(ta.write(time, out) for ta, out in zip(args[0], [new_state[1]]))
    else:
      for ta, out in zip(output_ta_t, output):
        ta[time.numpy()] = out

    return (time + 1, output_ta_t, new_state, state_ta_t)
  ...
  if return_all_states is not None:
  ...
  if in_graph_mode:
    ...
    final_states = tuple(ta.stack() for ta in state_info[1])
    # Restore some shape information
  ...
  if state_ta is not None:
    return (final_outputs, state_info[0], final_states)
  else:
    return (final_outputs, state_info[0])
