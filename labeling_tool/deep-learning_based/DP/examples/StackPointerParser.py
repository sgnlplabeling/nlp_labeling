...
def main():
    ...
    args_parser.add_argument('--patient', help='previous model patient')
    args_parser.add_argument('--decay', help='previous model decay')
    # hoon : bert
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for batch in range(1, num_batches + 1):
            ...
            # L2R
            loss_arc, loss_type, loss_cov, num = network.loss(word, char, pos, heads, stacked_heads, children,
                                                              sibling, stacked_types, previous, nexts, label_smooth,
                                                              skip_connect=skip_connect, mask_e=masks_e,
                                                              length_e=lengths_e, mask_d=masks_d,
                                                              length_d=lengths_d,
                                                              input_word_elmo=word_elmo, input_word_bert = word_bert)
        ...
        for batch in conllx_stacked_data.iterate_batch_stacked_variable(data_dev, batch_size, pos_embedding, type='dev', elmo=use_elmo, bert=use_bert):
            ...
            stacked_heads, children, sibling, stacked_types, skip_connect, previous, nexts, masks_d, lengths_d = input_decoder
            if use_elmo:
                word, char, pos, heads, types, masks_e, lengths, word_elmo, word_bert = input_encoder
                heads_pred, types_pred, _, _ = network.decode(word, char, pos, previous, nexts, stacked_heads, input_word_elmo=word_elmo,
                                                              mask_e=masks_e, mask_d=masks_d,length=lengths, beam=beam,
