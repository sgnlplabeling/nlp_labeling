...
def read_corpus_yield(path, max_chars=None, max_sent_len=20):
  """
  read raw text file
  :param path: str
  :param max_chars: int
  ...
  for i in perm:
    print("지금 몇번째 파일 하는중? ",i)
    data = []
    with codecs.open(path+"_%d"%(i)+'.txt', 'r', encoding='utf-8') as fin:
      for line in fin:
        data.append('<bos>')
        for token in line.strip().split():
          if max_chars is not None and len(token) + 2 > max_chars:
  ...
  if char2id is not None:
    ...
    for i, x_i in enumerate(x):
      for j, x_ij in enumerate(x_i):
        ...
        if x_ij == '<bos>' or x_ij == '<eos>':#여기가 거기임.
          batch_c[i][j][1] = char2id.get(x_ij)
          batch_c[i][j][2] = eow_id
        else:
          wordlen = -1
          for k, c in enumerate(x_ij):#######################################여기 바꾼다 뒤에 / 자른다 글자만 넣을거야.
            if(c=='/'):
              wordlen=k
              break
...
def create_batches(entiretxt, batch_size, word2id, char2id, config, perm=None, shuffle=True, sort=True, use_cuda=True):
  """
  ...
  for x in entiretxt:
    ...
    print("create 배치중",nbatch)
    start_time = time.time()
    for i in range(nbatch):
      #print(i)
      if(i%1000==0):
        print('%d / %d 번 완료하였다.'%(i,nbatch))
        print(' 경과된 시간 : %.2f'%(time.time() - start_time))
        start_time = time.time()
...
class Model(nn.Module):
  ...
  def forward(self, word_inp, chars_inp, mask_package):
    ...
    token_embedding = self.token_embedder(word_inp, chars_inp, (mask_package[0].size(0), mask_package[0].size(1)))######이건 아마 입력단 처리인듯. cnn 통과시켜 인풋 만듬
    ...
    if encoder_name == 'elmo':
      ...
      encoder_output = self.encoder(token_embedding, mask)#이게 엘모인듯.
    ...
    return self.classify_layer(forward_x, forward_y), self.classify_layer(backward_x, backward_y)#여기서 학습 ... 아마 사이즈만 잘맞으면 될거같은데.

  def save_model(self, path, save_classify_layer):
    #혹시나 몰라서 저장해놓고(백업용)
    torch.save(self.token_embedder.state_dict(), os.path.join(path, 'pkl_backup/token_embedder'+str(self.savecnt)+'.pkl'))
    torch.save(self.encoder.state_dict(), os.path.join(path, 'pkl_backup/encoder'+str(self.savecnt)+'.pkl'))
    if save_classify_layer:
      torch.save(self.classify_layer.state_dict(), os.path.join(path, 'pkl_backup/classifier'+str(self.savecnt)+'.pkl'))
...
def train_model(epoch, opt, model, optimizer,
  ...
  for w, c, lens, masks in train:
    ...
    total_loss += loss_forward.data###########################################################여기 수정하였다~~~~~~~~~~~~~~~~~~~~~~~~
    n_tags = sum(lens)
    total_tag += n_tags
    loss.backward()
    ...
    optimizer.step()
    if cnt * opt.batch_size % 8192 == 0:
      logging.info("Epoch={} iter={} lr={:.6f} train_ppl={:.6f} time={:.2f}s".format(
        epoch, cnt, optimizer.param_groups[0]['lr'],
        torch.exp(total_loss / total_tag), time.time() - start_time
      ))
      start_time = time.time()

    if cnt % opt.eval_steps == 0 or cnt % 500==0: #len(train_w) == 0:
      if valid is None:
        #tcpu=(total_loss / total_tag) ########여기 바꿈.
        #print(tcpu)
        train_ppl = torch.exp(total_loss / total_tag)
...
def train():
  ...
  cmd.add_argument('--cont', default=1, type=int, help='if continue 1 load previous model')###################불러올려면 여기 저장
  ...
  cmd.add_argument('--save_classify_layer', default=True, action='store_true',
...
def test():
  ...
  cmd.add_argument('--gpu', default=0, type=int, help='use id of gpu, -1 if cpu.')
