...
def get_embedding(unseen_words, embedd_dict):
    unseen_emb_dict = {word: embedd for word, embedd in list(embedd_dict.items())
...
def save_to_file(word_alphabet, model, unseen_emb_dict):
    with open(os.path.join(path, 'embedding.txt'), 'w') as f:
        for word, idx in list(word_alphabet.items()):
        ...
        for word, embedding in list(unseen_emb_dict.items()):
