...
class Alphabet(object):
    def __init__(self, name, pos_embedding=None, defualt_value=False, keep_growing=True, singleton=False):
        self.__name = name

        self.instance2index = {}
        self.instances = []
        self.pos_embedding = pos_embedding
        self.default_value = defualt_value    # 0 for UNK
    ...
    def add(self, instance):
        #if self.pos_embedding != 1:    # 2 & 4일 경우
        if isinstance(instance, list):
            for inst in instance:
                if inst not in self.instance2index:
                    self.instances.append(inst)
                    self.instance2index[inst] = self.next_index
                    self.next_index += 1

        elif instance not in self.instance2index:
    ...
    def is_singleton(self, id):
        if self.singletons is None:
            ...
            #return False if sum([elem in self.singletons for elem in id]) == 0 else True
        else:
    ...
    def items(self):
        return list(self.instance2index.items())
    ...
    def enumerate_items(self, start):
        ...
        return list(zip(list(range(start, len(self.instances) + self.offset)), self.instances[start - self.offset:]))
    ...
    def get_content(self):
        if self.singletons is None:
            return {'instance2index': {str(key): value for key, value in list(self.instance2index.items())},
                    'instances': [str(item) for item in self.instances]}
        else:
            return {'instance2index': {str(key): value for key, value in list(self.instance2index.items())},
                    'instances': [str(item) for item in self.instances],
                    'singletons': [str(item) for item in self.singletons]}

    def __from_json(self, data):
        self.instances = data["instances"]
        self.instance2index = data["instance2index"]
        if 'singletons' in data:
            self.singletons = set(data['singletons'])
        else:
    ...
    def save(self, output_directory, name=None):
        ...
        try:
            ...
            json.dump(self.get_content(),
                      open(os.path.join(output_directory, saving_name + ".json"), 'w', encoding="utf-8"), indent=4, ensure_ascii=False)
