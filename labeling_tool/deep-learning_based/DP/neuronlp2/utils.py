...
def load_embedding_dict(embedding, embedding_path, normalize_digits=False):
    ...
    if embedding == 'word2vec':    # 얘도 tag없음!
        # loading word2vec
        ...
        with gzip.open(embedding_path, 'r') as file:
            ...
            for line in file:
                ...
                try:
                    ...
                    continue
        return embedd_dict, embedd_dim
    elif embedding == 'polyglot':    # 64차원, 121004개의 단어, 어절 단위!postag 없음!
        ...
        with open(embedding_path, 'rb') as file:
            for line in file:
                ...
                tokens = line.split()
                if embedd_dim != -1 and embedd_dim != len(tokens)-1:
                    skipped += 1
                    continue
                if embedd_dim < 0:
                    embedd_dim = len(tokens) - 1
        ...
        raise ValueError("embedding should choose from [word2vec, senna, glove, sskip, polyglot, NNLM]")
