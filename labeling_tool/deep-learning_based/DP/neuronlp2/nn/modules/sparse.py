...
import torch.nn.functional as F # 2to3???
...
class Embedding(nn.Module):
    ...
    def reset_parameters(self, init_embedding):
        if init_embedding is None:
            ...
            self.weight.data.copy_(init_embedding)
    ...
    def forward(self, input):
        ...
        # 2to3???
        return F.embedding(input, self.weight, padding_idx, self.max_norm, self.norm_type, self.scale_grad_by_freq, self.sparse).view(output_size)
        """
