...
class myLSTMCell(RNNCellBase):
    ...
    def reset_noise(self, batch_size):
        if self.training:
            if self.p_in:
                ...
                # self.noise_in = Variable(noise.bernoulli_(1.0 - self.p_in)) # hoon : test
            else:
                self.noise_in = None
            ...
            if self.p_hidden:
                ...
                # self.noise_hidden = Variable(noise.bernoulli_(1.0 - self.p_hidden)) # hoon : test
            else:
                self.noise_hidden = None
        else:
            self.noise_in = None
    ...
    def forward(self, input, hx=None):
        ...
        if self.noise_in is not None:
            input = input * self.noise_in

        hx2, cx = hx
        if self.noise_hidden is not None:
            hx2 = hx2 * self.noise_hidden
