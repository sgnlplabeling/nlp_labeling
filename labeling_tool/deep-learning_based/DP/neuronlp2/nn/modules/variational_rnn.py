...
from neuronlp2.nn.modules import rnn as rnn
...
def default_initializer(hidden_size):
    ...
    return forward    # NOTE: returns a function
...
class VarMaskedFastLSTM(VarMaskedRNNBase):
    ...
    def __init__(self, *args, **kwargs):
        super(VarMaskedFastLSTM, self).__init__(rnn.myLSTMCell, *args, **kwargs)
        self.lstm = True
...
class VarLSTMCell(VarRNNCellBase):
    ...
    def __init__(self, input_size, hidden_size, bias=True, p=(0.5, 0.5), initializer=None):
        ...
        self.hidden_size = hidden_size
        self.bias = bias
        self.weight_ih = Parameter(torch.Tensor(4, input_size, hidden_size))    # QUESTION: 4?
        self.weight_hh = Parameter(torch.Tensor(4, hidden_size, hidden_size))
        if bias:
            self.bias_ih = Parameter(torch.Tensor(4, hidden_size))
            self.bias_hh = Parameter(torch.Tensor(4, hidden_size))
        else:
            self.register_parameter('bias_ih', None)    # NOTE: nn.module.register_parameter에서 등록 안 시키기!
            self.register_parameter('bias_hh', None)
    ...
    def reset_parameters(self):
        for weight in self.parameters():
            if weight.dim() == 2:    # QUESTION: what is this case?
                weight.data.zero_()
            else:
                self.initializer(weight.data)
