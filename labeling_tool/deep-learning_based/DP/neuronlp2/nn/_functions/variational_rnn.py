...
import torch
from neuronlp2.nn._functions import rnnFusedPointwise as fusedBackend
from torch.nn import functional as F

# RNN, LSTM, GRU
# variation means applying dropout, mostly
...
# NOTE: cell is a function, and weights are given as function parameters
...
def VarFastLSTMCell(input, hidden, w_ih, w_hh, b_ih=None, b_hh=None, noise_in=None, noise_hidden=None):
    if noise_in is not None:    # dropout
        input = input * noise_in
...
def VarMaskedRecurrent(reverse=False):
    def forward(input, hidden, cell, mask):
        output = []
        steps = list(range(input.size(0) - 1, -1, -1)) if reverse else list(range(input.size(0)))
        for i in steps:
...
def StackedRNN(inners, num_layers, lstm=False):
    ...
    def forward(input, hidden, cells, mask):
        ...
        if lstm:
            next_h, next_c = list(zip(*next_hidden))
            next_hidden = (
