...
class Vocabulary(object):
	...
	def __init__(self, word_filename, char_word_filename, validate_file=False):
		'''
		...
		self._bos = -1
		self._eos = -1
		self._char_word_filename = char_word_filename
...
class UnicodeCharsVocabulary(Vocabulary):
	...
	def __init__(self, word_filename, char_word_filename, max_word_length, **kwargs):
		super(UnicodeCharsVocabulary, self).__init__(word_filename, char_word_filename, **kwargs)
		...
		# < EOS >
		# < NUM >
		self.bos_char = 3  # <begin sentence>
		self.eos_char = 6  # <end sentence>
		self.bow_char = 4  # <begin word>
		self.eow_char = 5  # <end word>
		self.pad_char = 1  # <padding>
		self.hanja_char = 0

		self.char2idx = {}
		with open(char_word_filename) as f:
	...
	def _convert_word_to_char_ids(self, word):
		...
		for k, char in enumerate(word, start=1):
			if char not in self.char2idx:
				char = "<UNK>"
			elif hanja.is_hanja(char):
				char = "<HANJA>"
			chr_id = self.char2idx[char]
			code[k] = chr_id

		code[len(word) + 1] = self.eow_char
	...
	def encode_chars(self, raw_sentence, reverse=False, split=True):
		'''
...
class LMDataset(object):
	...
	def __init__(self, filepattern, vocab, reverse=False, test=False,
		...
		# self._all_shards = []
		# filenames = os.listdir(filepattern)
		# for filename in filenames:
		#     self._all_shards.append(os.path.join(filepattern, filename))
	...
	def _load_shard(self, shard_name):
		...
		for sentence in sentences:
			char_id = self.vocab.encode_chars(sentence, self._reverse)
			id = self.vocab.encode(sentence, self._reverse)
			if len(id) == len(char_id):
