...
parser.add_argument('-boot_iter', type=int, nargs=1, default=0)
parser.add_argument('-s', action='store_true')
parser.add_argument('-g', action='store_true')
parser.add_argument('-i', action='store', default='', type=str)
parser.add_argument('-o', action='store', default='', type=str)
parser.add_argument('-na', action='store_true')
args = parser.parse_args()
BOOT_ITER = args.boot_iter
SAVE_MODEL = args.s
GENERATE_FULL = args.g
FULL_IN_FILENAME = args.i.strip()
FULL_OUT_FILENAME = args.o.strip()
NON_ACTIVE = args.na
...
def generate(all_active_sents, all_y_active, unlabeled_orisents,
             unlabeled_nesents, unlabeled_sents, X_unlabeled, model):
    anm = ''
    a = ''
    m = ''
    active_dict = ner_generate.gen_active_dict(all_active_sents, all_y_active)
    print 'making predictions for unlabeled full data'
    y_pred_unlabled, y_pred_prob \
        = model.make_prediction(X_unlabeled, remove_all_o=False, min_conf=-1.0, link_pos=None)
    y_pred_including_act, y_prob_including_act, unlabeled_orisents_act,\
    unlabeled_nesents_act, X_unlabeled_act, y_pred_act, y_prob_act, unlabeled_orisents_ml,\
    unlabeled_nesents_ml, X_unlabeled_ml, y_pred_ml, y_prob_ml \
        = ner_generate.separate(active_dict, unlabeled_orisents, unlabeled_nesents,
                                unlabeled_sents, X_unlabeled, y_pred_unlabled, y_pred_prob)
    if st.PROB_OUT == True:
        if len(FULL_OUT_FILENAME) == 0:
            anm = st.SELF_LABELED_PATH+'actNmachine_' + MODEL_NUMBER + '.txt'
            a = st.SELF_LABELED_PATH+'active_' + MODEL_NUMBER + '.txt'
            m = st.SELF_LABELED_PATH+'machine_' + MODEL_NUMBER + '.txt'
            print 'writing : ' + st.SELF_LABELED_PATH+ 'actNmachine_'+MODEL_NUMBER +'.txt'
            utils.write_result_from_ft(unlabeled_orisents, unlabeled_nesents,
                                       X_unlabeled, y_pred_including_act,
                                       st.SELF_LABELED_PATH+'actNmachine_'+MODEL_NUMBER +'.txt',
                                       yprob=y_prob_including_act)
            print 'writing : ' + st.SELF_LABELED_PATH+ 'active_' + MODEL_NUMBER + '.txt'
            utils.write_result_from_ft(unlabeled_orisents_act, unlabeled_nesents_act,
                                       X_unlabeled_act, y_pred_act,
                                       st.SELF_LABELED_PATH+'active_'+MODEL_NUMBER+'.txt',
                                       yprob=y_prob_act)
            print 'writing : ' + st.SELF_LABELED_PATH+ 'machine_' + MODEL_NUMBER + '.txt'
            utils.write_result_from_ft(unlabeled_orisents_ml, unlabeled_nesents_ml,
                                       X_unlabeled_ml, y_pred_ml,
                                       st.SELF_LABELED_PATH + 'machine_' + MODEL_NUMBER + '.txt',
                                       yprob=y_prob_ml)
        else:
            anm = FULL_OUT_FILENAME
            print 'writing : ' +FULL_OUT_FILENAME
            utils.write_result_from_ft(unlabeled_orisents, unlabeled_nesents,
                                       X_unlabeled, y_pred_including_act,
                                       FULL_OUT_FILENAME,
                                       yprob=y_prob_including_act)
        ...
        m = st.SELF_LABELED_PATH + 'machine_' + MODEL_NUMBER + '.txt'
        print 'writing : ' +  st.SELF_LABELED_PATH+'actNmachine_' + MODEL_NUMBER + '.txt'
        utils.write_result_from_ft(unlabeled_orisents,
                                   unlabeled_nesents, X_unlabeled,
                                   y_pred_including_act,
                                   st.SELF_LABELED_PATH+'actNmachine_'+MODEL_NUMBER+'.txt')
        print 'writing : ' + st.SELF_LABELED_PATH+'active_'+ MODEL_NUMBER+'.txt'
        utils.write_result_from_ft(unlabeled_orisents_act,
                                   unlabeled_nesents_act, X_unlabeled_act, y_pred_act,
                                   st.SELF_LABELED_PATH+'active_'+MODEL_NUMBER+'.txt')
        print 'writing : ' +  st.SELF_LABELED_PATH+'machine_'+MODEL_NUMBER+'.txt'
        utils.write_result_from_ft(unlabeled_orisents_ml, unlabeled_nesents_ml,
                                   X_unlabeled_ml, y_pred_ml,
...
def main() :
    ...
    bagging_model = BaggingModel(BAGGING_MODEL_PATH, BAGGING_MODEL_NAME,
                                 num_of_comp_mds=st.NUM_BAGGING_MODEL,
                                 boot_sample_size=st.BOOTSTRAP_SAMPLE_SIZE / st.NUM_BAGGING_MODEL,
                                 X_labeled=X_train,
                                 y_labeled=y_train)
    ...
    if st.SELF_ITER_N == 1 :
        ...
        basic_CRF.add_n_train_CRF(X_basiccrf, y_basiccrf,
    ...
    if GENERATE_FULL is True :
        anm,a,m = generate(act_sents, y_act,unlabeled_orisents,
                           unlabeled_nesents, unlabeled_sents,
                           X_unlabeled, basic_CRF)
    ...
    if NON_ACTIVE == False :
        ...
        if st.SELF_ITER_N ==1:
            ...
            for boot_sub_iter in range(1, st.SELF_ITER_N):
                st.write_log('sub-iter : ' + str(boot_sub_iter) + '/' + str( st.SELF_ITER_N-1),
                             open=True,
                             std_print=True)
                print 'sub : Training Basic CRF'
                basic_CRF.temp_add_n_train_CRF(X_basiccrf, y_basiccrf, boot_sub_iter)
                if st.ALL_TEST is True:
                    tester.eval_prediction(basic_CRF.make_prediction(tester.X_test,
                                                                     remove_all_o=False)[0])
                y_pred_u = basic_CRF.make_prediction(X_unlabeled_now,
                                                     remove_all_o=True,
