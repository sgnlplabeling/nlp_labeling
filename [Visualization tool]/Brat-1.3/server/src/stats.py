...
def get_statistics(directory, base_names, use_cache=True):
    ...
    directory=directory.decode('utf-8').encode('utf-8')
    try:
        if (not isfile(cache_file_path)
            ...
            generate = False
            try:
                with open(cache_file_path.decode('utf-8').encode('utf-8'), 'rb') as cache_file:
    ...
    if generate:
        ...
        try:
            with open(cache_file_path.decode('utf-8').encode('utf-8'), 'wb') as cache_file:
