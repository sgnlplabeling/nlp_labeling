...
var AnnotatorUI = (function($, window, undefined) {
    var AnnotatorUI = function(dispatcher, svg) {
      ...
      var fillSpanTypesAndDisplayForm = function(evt, spanText, span) {
    	...
    	//20171016 docId가 없는경우(읽기전용) 수정,추가,삭제 안되도록 @jhseong
     	 if (docType != 'edit') return;
...
//                        true, true);
      ...
      var onMouseUp = function(evt) {
        ...
        if (arcDragOrigin) {
          ...
          // 2017.11.16 number40 선택된 영역이 태그가능한 것인지 확인
          var rowIndexFrom = sel.anchorNode && $(sel.anchorNode.parentNode.parentNode).index();
          var rowIndexTo = sel.anchorNode && $(sel.focusNode.parentNode.parentNode).index();
          
          //2017.11.29 number40 임시 삭제
//          if($('.background>rect:eq('+rowIndexFrom+')').attr('class') == 'backgroundHighlight' || $('.background>rect:eq('+rowIndexTo+')').attr('class') == 'backgroundHighlight') {
//        	  alert("태그 불가능한 영역이 선택되었습니다.");
//        	  return;
//          }
...
*/
      ...
      var deleteSpan = function() {
    	...
    	dispatcher.post('messages', [[["annotation 설정이 삭제되었습니다.", 'comment']]]);
      ...
      var spanFormSubmit = function(evt, typeRadio) {
    	...
    	dispatcher.post('messages', [[["annotation 설정이 저장되었습니다.", 'comment']]]);
        return false;
      };
