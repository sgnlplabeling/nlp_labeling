...
<script type="text/javascript" src="/resources/js/page/data/domain/list.js"></script>
			...
			<div class="type_domain01 clear2">
				...
				<div class="cont_win domain_02">
					...
					<div class="mt_30">
						...
						<!-- 목록 검색 start -->
							<form id="searchForm" action="/data/domain/list.do?_format=json" method="post">
									...
									<c:forEach var="result" items="${list}" varStatus="status">
										<tr>
											<c:if test="${pre_docId!=result.docId}">
						                		...
						                		<td rowspan="${result.count}" title='<c:if test="${not empty result.domainPath}">
						                		${result.domainPath}/</c:if>${result.domain}'><div class="of_hidden">${result.domain}</div></td>
						                		<td rowspan="${result.count}" title="${result.subject}"><div class="of_hidden">${result.subject}</div></td>
					                		...
					                		<td>
												...
												<a href="javascript:fn_docDelete('${result.docId}');" class="btn_td btn_tbl_del" style="display:inline-block; width:70px;">문서삭제</a>
												</c:if>
												<c:if test="${not empty result.groupName}">
													<c:if test="${result.rabelStat != '처리중'}">
							                			...
							                			<a href="javascript:fn_recordDelete('${result.recordId}');" class="btn_td btn_tbl_del" style="display:inline-block; width:70px;">
							                			<c:if test="${result.groupName == 'namedentity'}">개체명</c:if>
							                			<c:if test="${result.groupName == 'syntactic'}">구문분석</c:if>
							                			<c:if test="${result.groupName == 'causation'}">인과관계</c:if>
							                			삭제</a>
